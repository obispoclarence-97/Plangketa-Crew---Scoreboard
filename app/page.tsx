...
export default function Page() {
  ...
  const toggleFullscreen = useCallback(async () => {
    try {
      ...
      // Fullscreen API blocked — fall back to layout-only fullscreen.
      setIsFullscreen((v) => !v)
    }
  }, [])
  ...
  const handleResetScore = useCallback(
    (side: 1 | 2) => {
      sb.changeScore(side, side === 1 ? -sb.score1 : -sb.score2)
    },
    [sb],
  )
  ...
  if (!sb.hydrated) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-background">
        <span className="animate-pulse-live font-display text-lg tracking-[0.3em] text-gold">
          LOADING SCOREBOARD…
        </span>
      </main>
    )
  ...
  return (
    <div ref={rootRef} className="min-h-screen bg-background">
      <main className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8 lg:gap-8">
        <ScoreboardHeader
        ...
        <MatchCard
          players={sb.players}
          raceTo={sb.raceTo}
          onChangeRaceTo={sb.changeRaceTo}
        ...
        {!isFullscreen ? (
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="flex flex-col gap-6">
              ...
              />
              <PlayersManager stats={sb.stats} onAddPlayer={sb.addPlayer} />
            </div>
          </div>
        ) : null}

        <footer className="pb-4 text-center font-display text-[10px] tracking-[0.3em] text-foreground/30">
          PLANGKETA CREW · DIGITAL BILLIARDS SCOREBOARD
        </footer>
      </main>
      ...
      <ConfirmDialog
        ...
        message="This clears all matches and restores the default player roster (Clarence, Zeke, Kyzer, Josh). This cannot be undone."
